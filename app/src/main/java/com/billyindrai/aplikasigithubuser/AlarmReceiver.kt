...
class AlarmReceiver : BroadcastReceiver() {
    ...
    companion object {
        const val TYPE_ALARM = "Alarm Reminder"
    ...
    private fun showAlarmNotification(context: Context, title: String, message: String, notifId: Int) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(alarmID, alarmName, NotificationManager.IMPORTANCE_DEFAULT)
            channel.enableVibration(true)
            channel.vibrationPattern = longArrayOf(1000, 1000, 1000, 1000, 1000)
            builder.setChannelId(alarmID)
    ...
    fun setAlarm(context: Context, type: String, time: String, message: String) {
        ...
        Toast.makeText(context, "Alarm Reminder Is ON", Toast.LENGTH_SHORT).show()
    }

    fun cancelAlarm(context: Context, typeAlarm: String) {
        val alarmManager = context.getSystemService(Context.ALARM_SERVICE) as AlarmManager
        val intent = Intent(context, AlarmReceiver::class.java)
